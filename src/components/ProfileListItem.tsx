...
import { getDirectConsoleUrl, getDirectConsoleUrlWithRegion, AWS_REGIONS } from "../lib/url-builder";
...
function getStageColor(stage: string): Color {
  ...
  if (lower.includes("uat") || lower.includes("qa") || lower.includes("test")) return Color.Orange;
  if (lower.includes("dev") || lower.includes("sandbox")) return Color.Green;
  if (lower.includes("security") || lower.includes("audit")) return Color.Purple;
...
function getStageIcon(stage: string): Icon {
  ...
  if (lower.includes("stag") || lower.includes("uat") || lower.includes("qa")) return Icon.Eye;
...
export function AccountListItem({
  ...
  if (isSelected) {
    accessories.push({ icon: { source: Icon.CheckCircle, tintColor: Color.Green }, tooltip: "Selected for multi-session" });
  }
  ...
  if (showUsageCount && usageCount > 0) {
    accessories.push({ tag: { value: `${usageCount}×`, color: Color.Purple }, tooltip: `Opened ${usageCount} time${usageCount > 1 ? "s" : ""} total` });
  }
  ...
  const handleOpen = async (url: string, shouldTrack = false) => {
    try {
      openUrl(url);
      if (shouldTrack) {
        await trackUsage(defaultProfile.ssoAccountId, defaultProfile.ssoRoleName, defaultProfile.region || "us-east-1");
        onUsageTracked();
      }
    } catch (e) {
  ...
  return (
    <List.Item
      ...
      icon={account.stage ? { source: getStageIcon(account.stage), tintColor: getStageColor(account.stage) } : Icon.Globe}
      keywords={[account.accountId, account.accountAlias || "", account.stage || "", ...account.roles.map((r) => r.ssoRoleName)]}
        ...
        <ActionPanel>
          ...
          <ActionPanel.Section title="Quick Open">
            <Action
              ...
              onAction={() => handleOpen(getDirectConsoleUrl(defaultProfile), true)}
            />
            <ActionPanel.Submenu
            ...
            >
              {AWS_REGIONS.map((r) => (
                <Action
                  ...
                  icon={r.value === defaultProfile.region ? Icon.CheckCircle : Icon.Circle}
                  onAction={() => handleOpen(getDirectConsoleUrlWithRegion(defaultProfile, r.value))}
                />
              ))}
            </ActionPanel.Submenu>
          </ActionPanel.Section>
          ...
          <ActionPanel.Section title="Multi-Session (up to 5)">
            <Action
              title={isSelected ? "Deselect for Multi-Session" : "Select for Multi-Session"}
